...
def draw_player_stats(output_video_frames,player_stats):
    ...
    for index, row in player_stats.iterrows():
        ...
        player_1_hand = row['player_1_last_hitting_hand']
        player_2_hand = row['player_2_last_hitting_hand']

        frame = output_video_frames[index]
        shapes = np.zeros_like(frame, np.uint8)

        width=350
        height=270 # Increased height for the extra row
        ...
        text = f"{player_1_hand}           {player_2_hand}"
