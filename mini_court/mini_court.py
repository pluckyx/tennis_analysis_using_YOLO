...
class MiniCourt():
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
        ...
        default_player_height = 1.85  # fallback for any track_id not 1 or 2
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            # player_bbox[x] is a dict {"bbox": [...], "keypoints": ...} — extract bbox before passing
            closest_player_id_to_ball = min(
                player_bbox.keys(),
                key=lambda x: measure_distance(
                    ball_position,
                    get_center_of_bbox(player_bbox[x]["bbox"] if isinstance(player_bbox[x], dict) else player_bbox[x])
                )
            )

            output_player_bboxes_dict = {}
            for player_id, data in player_bbox.items():
                # Extract just the bounding box from the data dict
                ...
                # Get Player height in pixels — only from frames where this player is visible
                frame_index_min = max(0, frame_num-20)
                frame_index_max = min(len(player_boxes), frame_num+50)
                bboxes_heights_in_pixels = []
                for i in range(frame_index_min, frame_index_max):
                    if player_id in player_boxes[i]:
                        p_data = player_boxes[i][player_id]
                        p_bbox = p_data["bbox"] if isinstance(p_data, dict) else p_data
                        bboxes_heights_in_pixels.append(get_height_of_bbox(p_bbox))
                ...
                mini_court_player_position = self.get_mini_court_coordinates(foot_position,
                                                                            ...
                                                                            player_heights.get(player_id, default_player_height)
                                                                            )
